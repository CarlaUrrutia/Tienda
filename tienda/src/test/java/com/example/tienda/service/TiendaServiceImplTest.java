...
class TiendaServiceImplTest {
    ...
    @BeforeEach
    void setUp() {
        tienda = new Tienda(1, "Tienda Central", "Av. Principal 123", null, "Política general", 5);
    }
    ...
    void getTiendaById_existente_retornaTienda() {
    ...
    void getTiendaById_noExistente_retornaNull() {
    ...
    void updateTienda_existente_actualiza() {
        Tienda datosNuevos = new Tienda(1, "Tienda Norte", "Calle 45", null, "Nueva política", 5);
        when(tiendaRepository.buscarPorId(1)).thenReturn(List.of(tienda));
        when(tiendaRepository.save(any(Tienda.class))).thenAnswer(inv -> inv.getArgument(0));

        Tienda resultado = tiendaService.updateTienda(1, datosNuevos);
    ...
    void updateTienda_noExistente_retornaNull() {
        when(tiendaRepository.buscarPorId(99)).thenReturn(List.of());
